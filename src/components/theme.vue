<template>
  <div class="theme-box">
    <Header :title="Htitle" class="header" :fixed="true">
      ...
      <mt-button slot="right" @click.native="$router.replace('/category')">
    ...
    <div class="panel">
      ...
      <div class="desc">
        <h3>{{title}}</h3>
        <h4>{{desc}}</h4>
        <p>{{$t('m.placeTitle')}}：{{place}}</p>
        <p>{{$t('m.timerTitle')}}：{{time}} {{startTime}}-{{overTime}}</p>
        <p>{{$t('m.peopelNum')}}：{{person}}</p>
      </div>
    </div>
  </div>
</template>
...
.panel {
  background: url("./../assets/imgs/theme-bg.png") top left no-repeat;
